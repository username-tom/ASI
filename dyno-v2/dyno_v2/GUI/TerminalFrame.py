# From https://stackoverflow.com/questions/63597533/how-to-add-a-console-to-a-tkinter-window
...
def compile_terminal_command(terminal_command, last_line_index):
    ...
    result = subprocess.run(command_summary, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=True,
                            env=os.environ)
...
def terminal_enter_key_callback():
    ...
    compiler_thread = threading.Thread(target=compile_terminal_command,
                                       args=(terminal_text[3:], terminal_listbox.size()))
    ...
    terminal_listbox.insert(tk.END,
                            terminal_text)  # Insert the terminal text(basically a new line in this case) to the terminal listbox.
...
terminal_listbox = tk.Listbox(terminal, bg='black', fg='white', highlightcolor='black', highlightthickness=0,
                              selectbackground='black', activestyle=tk.NONE)
