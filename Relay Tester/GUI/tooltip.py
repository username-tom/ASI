...
class ToolTip(tk.Toplevel):
    ...
    def __init__(
        ...
        **message_kwargs : tkinter `**kwargs` passed directly into the ToolTip
        ...
        tk.Toplevel.__init__(self, **parent_kwargs, master=self.widget)
    ...
    def on_enter(self, event) -> None:
        ...
        for m in self.monitors:
            if m.x <= event.x_root <= m.x + m.width < event.x_root + self.x_offset + self.winfo_width():
                x_bound = True
            if m.y <= event.y_root <= m.y + ctypes.windll.user32.GetSystemMetrics(17) < event.y_root + self.y_offset + self.winfo_height():
                y_bound = True
        if x_bound and y_bound:
            self.geometry(f"+{event.x_root - self.x_offset - self.winfo_width()}+{event.y_root - self.y_offset - self.winfo_height()}")
        elif x_bound:
            self.geometry(f"+{event.x_root - self.x_offset - self.winfo_width()}+{event.y_root + self.y_offset}")
        elif y_bound:
            self.geometry(f"+{event.x_root + self.x_offset}+{event.y_root - self.y_offset - self.winfo_height()}")
        else:
